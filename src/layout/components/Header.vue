<template>
  <header :class="Topclass">
    ...
    <nav>
      <div class="contentbox">
        <ul>
          <li v-for="(item, index) in menulists" :key="index"><button v-html="item.menutext" v-on:click="goToPage(item.link)"></button></li>
        </ul>
      </div>
    </nav>
    <b-modal id="login" class="modal" hide-footer>
      ...
      <div class="modalcontainer">
        <div class="loginform">
          ...
          <b-form-input id="pass" type="password" placeholder="비밀번호를 입력해 주세요"/>
        </div>
        <div class="btnwrap">
    ...
    <b-modal id="member" class="modal" hide-footer>
      ...
      <div class="modalcontainer">
        <b-form @submit="Joinmember">
          ...
          <b-form-group id="formid3" label="이메일 수신" label-for="mailing" description="※ 이메일 수신을 허락하면 독자 혜택을 받을 수 있어요">
            <b-form-radio value="Y" v-model="form.mail">예</b-form-radio>
            <b-form-radio value="N" v-model="form.mail">아니오</b-form-radio>
          </b-form-group>
          <b-form-group id="formid4" label="관심 분야 선태" label-for="checkedtype" description="※ 관심 분야를 선택하세요. 여러 개 선택할 수 있어요">
